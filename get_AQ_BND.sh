...
urlbase=http://thredds.silam.fmi.fi/thredds/dodsC/silam_glob_v5_7_1/files/SILAM-AQ-glob_v5_7_1_${basedate}00
#urlbase=http://silam.fmi.fi/thredds/dodsC/silam_glob_v5_7_1/files/SILAM-AQ-glob_v5_7_1_${basedate}00

#List form global v5_7_1 that is pesent in KAZ
...
for try  in `seq 0 10`; do
   ...
   #for hr in `seq 48 52` ; do
        ...
        URL="${urlbase}_`printf %03d ${hr}`.nc4"
        ## The command that fails, but leaves trace in threds logs, so I could grep your request from there
        wget -q "$URL?ncksparams=\"$bbox -v ${varlist}\"&email=$email" -O /dev/null 2>&1>/dev/null || true
#        exit
        #
        ...
        (ncks -O $bbox -v ${varlist} "$URL" ${outf}.tmp && ncatted -h $attcmd ${outf}.tmp && $ncks $compresscmd ${outf}.tmp $outf && rm ${outf}.tmp && echo  $outf done!) & 
